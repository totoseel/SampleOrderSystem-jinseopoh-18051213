...
모든 구현은 `/test-driven-development` Skill의 **Red-Green-Review 사이클**을 따른다.  
테스트보다 프로덕션 코드를 먼저 작성하는 것은 절대 금지다.

### 사이클별 절차

**각 클래스·기능 단위로 아래 사이클을 반복한다.**

1. **🔴 RED — Plan.md 작성 후 Orchestrator 검토 요청**
   - 구현할 동작 하나를 선정한다
   - Plan.md에 시나리오(Given/When/Then)·예상 실패 이유·구현 범위를 기록한다
   - Orchestrator에게 검토를 요청하고, 승인 전까지 테스트 코드를 작성하지 않는다
...
3. **✅ REVIEW — 자가 검토 후 Orchestrator 검토 요청**
   ...
   - `./gradlew test` 재실행으로 그린 상태를 유지한다
   - 변경된 파일·테스트 메서드명을 Orchestrator에게 보고하고, 승인 전까지 다음 사이클을 시작하지 않는다

### 페이즈 내 구현 흐름

```
인터페이스·추상 클래스 정의
  → 각 메서드/동작마다 TDD 사이클 (RED → GREEN → REVIEW)
  → 구현체 각 동작마다 TDD 사이클
  → 모든 사이클 완료 후 변경 파일 목록 보고
```
